...
def seed_demo_data(
    ...
    demo_events = [
        ...
        FinancialEventORM(
            ...
            amount=Decimal("8450.50"),
            direction=EventDirection.DEBIT.value,
            currency=Currency.INR.value,
            date_occurred=date(2026, 8, 22),
            category="Groceries",
            confidence=Decimal("0.99"),
            status=EventStatus.CONFIRMED.value,
            source="CSV Import",
            raw_evidence={"snippet": "POS DEBIT WHOLE FOODS MARKET -8,450.50 DR"},
        ),
        FinancialEventORM(
            user_id=current_user.id,
            ...
            direction=EventDirection.DEBIT.value,
            currency=Currency.INR.value,
            date_occurred=date(2026, 8, 15),
            category="Entertainment",
            confidence=Decimal("0.99"),
            status=EventStatus.CONFIRMED.value,
            source="CSV Import",
            raw_evidence={"snippet": "ACH DEBIT NETFLIX.COM -649.00 DR"},
        ),
        FinancialEventORM(
            user_id=current_user.id,
            ...
            source="SMS Import",
            raw_evidence={"snippet": "INR 15,000.00 debited for Gold's Gym on 10-08-2026"},
        ),
        FinancialEventORM(
            user_id=current_user.id,
            ...
            direction=EventDirection.CREDIT.value,
            currency=Currency.INR.value,
            date_occurred=date(2026, 8, 28),
            category="Freelance",
            confidence=Decimal("0.95"),
            status=EventStatus.CONFIRMED.value,
            source="CSV Import",
            raw_evidence={"snippet": "NEFT TRANSFER - CLIENT X - LOGO DESIGN +12,000.00 CR"},
        ),
    ]
